...
private class Xnoise.ExtDev.GenericPlayerMainView : PlayerMainView {

    public GenericPlayerMainView(PlayerDevice audio_player_device,
                                 Cancellable cancellable) {
        print("create generic player main view\n");
        base(audio_player_device, cancellable);
    }

    protected override string get_localized_name() {
        return _("External Player Device");
    }

    protected override PlayerTreeView? get_tree_view() {
        return new GenericPlayerTreeView(audio_player_device, cancellable);
    }
}
