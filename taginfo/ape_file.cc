...
void ApeFile::write_items(void)
{
    ...
    for(index = 0; index < count; index++)
    {
        ...
        //printf(wxT("Writing item %i   '%s' => '%s' at %08X"), index, item->key.c_str(), item->value.c_str(), m_File->Tell());

        if(item->value.empty())
            continue;

        const char* ValueBuf = item->value.c_str();//mb_str(wxConvUTF8); const wxWX2MBbuf
        if(!ValueBuf)
            continue;
        
        //printf(wxT("'%s' => '%s'"), item->key.c_str(), item->value.c_str());
        //int KeyLen = item->key.size();
        ...
        else
        {
            ...
            //printf(wxT("'%s' Size: %u   Length: %u"),
            //    item->value.c_str(), item->value.size(), item->value.Length());
        }
    }
}
...
{
    //printf("file length %u, %08x\n", apetag->get_file_length(), apetag->get_offset());
...
void ApeFile::read_header(void)
{
    ...
    //printf(wxT("file length is %u"), FileLength);
...
//    printf(wxT("Track size %u  length %s  bitrate %u"), FileLength, LenToString(length).c_str(), bitrate);
    ...
    //printf(wxT("Found footer tag footer version: %i  length: %i  items: %i  flags: %08x"),
    ...
    if(FileLength >= footer.length + sizeof(ApeHeaderFooter))
    {
        ...
        if(footer.magic[0] == APE_MAGIC_0 &&
            footer.magic[1] == APE_MAGIC_1)
        {
            have_header = true;

            if(footer.version != footer.version || footer.length != footer.length || footer.item_count != footer.item_count)
            {
                printf("footer header/footer data mismatch\n");
            }

            //printf(wxT("Found footer tag header version: %i  length: %i  items: %i  flags: %08x"),
            //      footer.version, footer.length, footer.item_count, footer.flags);
        }
    }
    ...
    //printf(wxT("Found a valid ape footer with %i items and %i bytes length"), footer.item_count, footer.length);
    string Value;
    string Key;
    int index;
    for(index = 0; index < (int) footer.item_count; index++)
    {
        ...
        {
            Value = string(CurBufPos, ValueLen);//wxString::From8BitData(CurBufPos, ValueLen);
        }
        else
        {
        ...
        //printf(wxT("Tag%i => Len: %i  Flags: %08x  Key: '%s'  Value: '%s'"),
